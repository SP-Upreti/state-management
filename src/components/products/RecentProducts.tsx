...
export default function RecentProducts() {
    ...
    const fetchRecentProducts = async () => {
        try {
            setIsLoading(true);
            setError(null);
            const response = await productsApi.getRecentProducts(10);
            const { products } = response.data.data;
            setProducts(products);
        } catch (err: any) {
            const message = err.response?.data?.message || 'Failed to fetch recent products';
            setError(message);
        } finally {
            setIsLoading(false);
        }
        ...
        <section className="py-12 ">
            <div className="max-w-screen-xl mx-auto px-4 lg:px-0">
                <div className="flex justify-between items-center gap-4 flex-wrap mb-8">
                    <h3 className="text-gray-800 text-2xl font-semibold sm:text-4xl">Recently Added Products</h3>
                </div>
